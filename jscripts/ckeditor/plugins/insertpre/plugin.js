...
CKEDITOR.plugins.add( 'insertpre',
	{
		...
		{
				myalign = 'left';
				if(CKEDITOR.config.direction == 'rtl') {
					myalign = 'right';
				}
				CKEDITOR.addCss(
					...
					'pre.codeblock:before { ' +
						"display: block; margin-bottom:1px; border-bottom: 1px solid #ccc;direction: "+CKEDITOR.config.direction+";text-align: "+myalign+" ; " +
					' } ' +
					'pre.phpblock:before { ' +
						"display: block; margin-bottom:1px; border-bottom: 1px solid #ccc;direction: "+CKEDITOR.config.direction+";text-align: "+myalign+" ; " +
					' } '
		...
		init : function( editor )
		{
			if ( CKEDITOR.config.insertpre_class )
			{
				CKEDITOR.addCss(
					'pre.codeblock:before { ' +
						"content: '"+editor.lang.insertpre.codeblock+"';" +
					' } ' +
					'pre.phpblock:before { ' +
						"content: '"+editor.lang.insertpre.phpblock+"';" +
					' } '
				);
			}
			...
			editor.ui.addButton && editor.ui.addButton( 'InsertPHP',
				{
					...
					icon : this.path + 'icons/insertprephp.png',
					command : 'insertprephp',
					toolbar: 'insert,99'
				} );
			editor.ui.addButton && editor.ui.addButton( 'InsertCode',
				{
					label : editor.lang.insertpre.title,
					icon : this.path + 'icons/insertprecode.png',
					command : 'insertpre',
					toolbar: 'insert,99'
				} );
			...
			if ( editor.contextMenu )
			{
				...
				editor.addMenuItem( 'insertprephp',
					{
						label : editor.lang.insertpre.edit,
						icon : this.path + 'icons/insertprephp.png',
						command : 'insertprephp',
				...
				editor.contextMenu.addListener( function( element )
				{
					...
					if ( element && !element.isReadOnly() && element.hasClass( editor.config.insertpre_class ) && element.hasClass('codeblock'))
						return { insertpre : CKEDITOR.TRISTATE_OFF };
					if ( element && !element.isReadOnly() && element.hasClass( editor.config.insertpre_class ) && element.hasClass('phpblock'))
						return { insertprephp : CKEDITOR.TRISTATE_OFF };
					return null;
				});
			}
				...
				return {
					...
					onOk : function()
					{
						if ( editor.config.insertpre_class )
							this.pre.setAttribute( 'class', editor.config.insertpre_class + ' codeblock' );
			...
			CKEDITOR.dialog.add( 'insertprephp', function( editor )
			{
				return {
					title : editor.lang.insertpre.title,
					minWidth : 540,
					...
					{
						if ( editor.config.insertpre_class )
							this.pre.setAttribute( 'class', editor.config.insertpre_class + ' phpblock' );
...
if (typeof(CKEDITOR.config.insertpre_style) == 'undefined')
	CKEDITOR.config.insertpre_style = 'background-color:#f7f7f7;border:1px solid #DDD;padding:10px;direction:ltr;text-align:left;';
